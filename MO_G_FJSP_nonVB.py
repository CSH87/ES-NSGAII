...
if __name__ == '__main__' :
    ...
    print("Evolutioin......") 
    evo = myEvolution(problem, num_of_generations=5000, num_of_individuals=1000, mutation_schedule=[[0,120],[100,100],[200,80],[300,60],[400,40],[500,20],[600,10]])
    start = time.time()
    evol = evo.evolve()
    end = time.time()
    print("Total executed time : " + str(start-end))
    func=[]
    feature=[]
...
#%% testing
    ...
    makespan, transfer_time, transportation_time, energy, makespan_index, schedule_results, schedule_results_v2 = objective.Calculate(feature[-1][0])
    objective.check_schedule(schedule_results)
    objective.plot_gantt(feature[-1][0])
