...
import random
import numpy as np
from Class_define import myProblem
from Class_define import myEvolution
#%%
path = 'C:\\Users\\acanlab\\Desktop\\sihan\\TextData\\Monaldo\\Fjsp\\Job_Data\\Brandimarte_Data\\Text\\Mk01.fjs'
f = open(path, 'r')
line_cnt=0
index_of_map=0
total_operation=0
...
for line in f:
    ...
    if line_cnt==0:
        ...
        avg_machine_per_operation = int(line_split[2])
        job_machine_operation_map = [[]for _ in range(N_jobs)]
        operation_arr = []
        machine_distance = [[0]*N_machines]*N_machines
    elif line_cnt>=1 and line_cnt<1+N_machines:
        #print(cnt)
        machine_distance[line_cnt-1] = [int(d) for d in line_split]
    else:
